...
const OrderSummaryCard = () => {
  ...
  return (
    <div className="bg-[var(--color-surface)] rounded-[20px] p-8 shadow-sm h-fit sticky top-24">
      ...
      <div className="space-y-6 mb-8">
        {cartItems.map((item) => (
          <div key={item.variantId || item.id} className="flex gap-4">
            <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-[var(--color-muted-bg)] flex-shrink-0">
              <Image
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="font-semibold text-[var(--color-text-primary)] text-sm line-clamp-1">
                    {item.name}
                  </h4>
                ...
                <div className="text-sm font-bold text-[var(--color-text-primary)] flex items-center">
                  <Tk_icon size={12} className="mr-1" />
                  {(item.price * item.quantity).toFixed(2)} 
                  <span className="text-[var(--color-text-secondary)] font-normal ml-1">x{item.quantity}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-3 text-sm">
        ...
        {discountAmount > 0 && (
          <div className="flex justify-between text-[var(--color-text-primary)]">
            <span>Discount</span>
            <span className="font-medium flex items-center text-[var(--color-accent-green)]">
        ...
        <div className="flex justify-between text-[var(--color-text-primary)]">
          <span>Shipping</span>
          <span className={`font-medium ${isShippingFree ? 'text-red-500' : ''}`}>
            {isShippingFree ? 'Free' : (
        ...
        <div className="flex justify-between text-base font-bold text-[var(--color-text-primary)] pt-4">
          <span>Total</span>
