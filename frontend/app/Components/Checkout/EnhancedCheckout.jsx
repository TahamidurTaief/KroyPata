          ...
          {[1, 2, 3].map((stepNum) => (
            <div key={stepNum} className="flex items-center">
              <div className={`
                ...
                ${step >= stepNum ? 'bg-[var(--primary)] text-white' : 'bg-[var(--muted)] text-[var(--muted-foreground)]'}
              `}>
                {stepNum}
              </div>
              <span className={`ml-2 text-sm ${step >= stepNum ? 'text-[var(--primary)]' : 'text-[var(--muted-foreground)]'}`}>
                {stepNum === 1 && 'Shipping'}
                {stepNum === 2 && 'Review'}
                {stepNum === 3 && 'Complete'}
              </span>
              {stepNum < 3 && <div className="w-12 h-px bg-[var(--border)] ml-4" />}
            </div>
          ))}
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            {requiresSplitShipping && (
              <div className="bg-[#fefce8] border border-[#fef08a] rounded-lg p-4">
                <div className="flex items-start">
                  <svg className="w-5 h-5 text-[#facc15] mt-0.5 mr-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                  <div>
                    <h4 className="text-sm font-medium text-[#854d0e]">Split Shipping Required</h4>
            ...
            {freeShippingEligible && (
              <div className="bg-[#f0fdf4] border border-[#bbf7d0] rounded-lg p-4">
                <div className="flex items-center">
                  <svg className="w-5 h-5 text-[#4ade80] mr-3" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg">
              <div className="p-4 border-b border-[var(--border)]">
                <h3 className="text-lg font-semibold text-[var(--foreground)]">Select Shipping Method</h3>
              </div>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-start space-x-3">
                          ...
                          />
                          <div>
                            <h4 className="font-medium text-[var(--foreground)]">{method.name}</h4>
                            {method.description && (
                        ...
                        <div className="text-right">
                          ...
                          {method.is_free_shipping_rule && (
                            <div className="text-xs text-[#16a34a] mt-1">Free Shipping Applied</div>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg">
              <div className="p-4 border-b border-[var(--border)]">
                <h3 className="text-lg font-semibold text-[var(--foreground)]">Shipping Information</h3>
              </div>
              <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-[var(--border)] rounded-md focus:ring-[var(--primary)] focus:border-[var(--primary)] bg-[var(--background)] text-[var(--foreground)]"
                    required
                  />
                </div>
                <div>
            ...
            <div className="flex justify-between">
              <button
                onClick={onCancel}
                className="px-6 py-2 border border-[var(--border)] rounded-md text-[var(--muted-foreground)] hover:bg-[var(--muted)] transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="px-6 py-2 bg-[var(--primary)] text-white rounded-md hover:bg-[var(--primary)]/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Continue to Review
              </button>
            </div>
        ...
        {step === 2 && (
          ...
          >
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg">
              <div className="p-4 border-b border-[var(--border)]">
                <h3 className="text-lg font-semibold text-[var(--foreground)]">Order Review</h3>
              </div>
              <div className="p-4">
                {/* Cart Items */}
                <div className="space-y-3 mb-6">
                  {cartItems?.map((item, index) => (
                    <div key={index} className="flex items-center space-x-4 p-3 bg-[var(--muted)] rounded-md">
                      <div className="flex-1">
                        <h4 className="font-medium text-[var(--foreground)]">{item.name || `Product ${item.product_id}`}</h4>
                      ...
                      <div className="text-lg font-medium text-[var(--foreground)] flex items-center">
                ...
                <div className="border-t border-[var(--border)] pt-4 mb-6">
                  {appliedCoupon ? (
                    <div className="flex items-center justify-between p-3 bg-[#f0fdf4] border border-[#bbf7d0] rounded-md">
                      <div className="flex items-center">
                        <svg className="w-5 h-5 text-[#22c55e] mr-2" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                        </svg>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          ...
                          className="text-[#16a34a] hover:text-[#15803d] text-sm"
                        >
                          Remove
                        </button>
                      </div>
                    ...
                    <div className="flex space-x-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 border border-[var(--border)] rounded-md focus:ring-[var(--primary)] focus:border-[var(--primary)] bg-[var(--background)] text-[var(--foreground)]"
                      />
                      <button
                        onClick={handleApplyCoupon}
                        disabled={loading || !couponCode.trim()}
                        className="px-4 py-2 bg-[var(--primary)] text-white rounded-md hover:bg-[var(--primary)]/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Apply
                      </button>
                    </div>
                ...
                {checkoutData && (
                  <div className="space-y-2 pt-4 border-t border-[var(--border)]">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium flex items-center text-[var(--foreground)]"><Tk_icon size={16} className="mr-1" />{checkoutData.subtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[var(--muted-foreground)]">Shipping</span>
                      <span className="font-medium flex items-center text-[var(--foreground)]">
                    ...
                    <div className="flex justify-between text-lg font-semibold pt-2 border-t border-[var(--border)] text-[var(--foreground)]">
                      <span>Total</span>
            ...
            <div className="flex justify-between">
              ...
              <button
                ...
                className="px-6 py-2 bg-[#16a34a] text-white rounded-md hover:bg-[#15803d] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
              >
                {loading && (
        ...
        {step === 3 && (
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-[var(--foreground)] mb-2">Order Placed Successfully!</h2>
            <p className="text-[var(--muted-foreground)] mb-6">
              Thank you for your order. You'll receive a confirmation email shortly.
            </p>
            <button
              onClick={() => window.location.href = '/orders'}
              className="px-6 py-3 bg-[var(--primary)] text-white rounded-md hover:bg-[var(--primary)]/90 transition-colors"
            >
              View My Orders
            </button>
          </motion.div>
